...
이 파일은 이 템플릿에서 파생된 앱의 원래 바이브코딩 작업공간에 사용하는, 복제 직후의
빈 초기 상태다. 회사 Coolify·프로비저너 운영, 원본 템플릿 유지보수 또는 BMAD가 제외된
검토·배포용 인계 저장소에는 적용하지 않는다. 앱 코드 변경을 시작하기 전에 경로를 선택하고,
